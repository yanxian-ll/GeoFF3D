...
OUTPUT_PATH="${2:-$ROOT_DIR/experiments/benchmarking/slrf/vggt_omega/$SCENE_NAME}"
shift $(( $# >= 2 ? 2 : 1 ))

exec python3 "$ROOT_DIR/scripts/run_slrf.py" \
  model=vggt_omega \
  scene_dir="$SCENE_DIR" \
  checkpoint="${VGGT_OMEGA_CHECKPOINT:-${CHECKPOINT:-$ROOT_DIR/experiments/uav_training/vggt_omega_8v_4d_8ipg_2g/checkpoint-best.pth}}" \
