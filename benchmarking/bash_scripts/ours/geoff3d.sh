...
OUTPUT_PATH="${2:-$ROOT_DIR/experiments/benchmarking/slrf/geoff3d/$SCENE_NAME}"
shift $(( $# >= 2 ? 2 : 1 ))

exec python3 "$ROOT_DIR/scripts/run_slrf.py" \
  model=geoff3d \
  scene_dir="$SCENE_DIR" \
  checkpoint="${GEOFF3D_CHECKPOINT:-${CHECKPOINT:-$ROOT_DIR/checkpoints/geoff3d/checkpoint-best.pth}}" \
