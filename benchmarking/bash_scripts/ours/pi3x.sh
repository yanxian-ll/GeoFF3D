...
OUTPUT_PATH="${2:-$ROOT_DIR/experiments/benchmarking/slrf/pi3x/$SCENE_NAME}"
shift $(( $# >= 2 ? 2 : 1 ))

exec python3 "$ROOT_DIR/scripts/run_slrf.py" \
  model=pi3x \
  scene_dir="$SCENE_DIR" \
  checkpoint="${PI3X_CHECKPOINT:-${CHECKPOINT:-$ROOT_DIR/experiments/uav_training/pi3x_finetuning_8v_4d_8ipg_2g/checkpoint-best.pth}}" \
