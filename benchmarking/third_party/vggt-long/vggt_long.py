...
# Put VGGT-Long's local base_models first.
# Otherwise `from vggt...` may resolve to another VGGT package
# from VGGT-SLAM, site-packages, or an existing PYTHONPATH entry.
if base_models_path in sys.path:
    sys.path.remove(base_models_path)
sys.path.insert(0, base_models_path)
...
from local_weights import apply_local_weight_paths
...
class VGGT_Long:
    ...
    def run(self):
        ...
        self.processing_time_seconds = time.perf_counter() - processing_time_start
        print(
            "Processing time after model weights loaded "
            f"(excluding final merge/cleanup): {self.processing_time_seconds:.6f}s"
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, required=False, default='./configs/base_config.yaml',
                        help='config path')
    parser.add_argument('--timing_path', type=str, required=False, default=None,
                        help='Optional JSON path for processing time metadata')
    args = parser.parse_args()

    config = apply_local_weight_paths(load_config(args.config))
    ...
    vggt_long.run()
    processing_time_seconds = float(vggt_long.processing_time_seconds or 0.0)
    timing_payload = {
        ...
        "start_event": "after_model_weights_loaded",
        "end_event": "before_temp_cleanup_and_final_pointcloud_merge",
        "excluded": ["weight_loading", "final_pointcloud_merge", "temp_cleanup"],
        "note": (
            "VGGT-Long loop retrieval currently runs before the VGGT model load "
            "and is not included in this model-processing timer."
        ),
        "num_images": int(len(vggt_long.img_list) if vggt_long.img_list is not None else 0),
        "num_chunks": int(len(vggt_long.chunk_indices) if vggt_long.chunk_indices is not None else 0),
        "loop_enable": bool(vggt_long.loop_enable),
        "num_loop_pairs": int(len(vggt_long.loop_list)),
    }
    print(f"Processing time (excluding final merge/cleanup): {processing_time_seconds:.6f}s")
    if args.timing_path:
