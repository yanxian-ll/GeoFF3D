...
class MapFree_Multi(BaseMultiViewDataset):
    ...
    def path2imgid(self, subscene, filename):
        # st()
        first_seq_id = int(subscene[6:]) # dense_
        first_frame_id = int(filename[6:-4])
        return [first_seq_id, first_frame_id]

    def _load_data(self):
        # cache_file = f"{self.ROOT}/cached_metadata_50_col_only_fix.h5"
        # cache_file = f"cached_metadata_50_col_only_fix.h5"
        cache_file = f"{self.ROOT}/cached_metadata_50_col_only_fix.pkl"
        # cache_file = "cached_metadata_50_col_only.h5"
        # st()
        ...
        # if False:
            ...
            # ! save to pkl for /oss loading
            # with open(f"{self.ROOT}/cached_metadata_50_col_only_fix.pkl", 'wb') as f:
            #     pickle.dump(dict(
            #         scenes=self.scenes,
            ...
            # with open(f"{self.ROOT}/cached_metadata_50_col_only_fix.pkl", "rb") as f:
            with open(cache_file, "rb") as f:
                hf = pickle.load(f)
                self.scenes = hf['scenes']
                self.sceneids = hf["sceneids"][:]
                self.scope = hf["scope"][:]
                self.video_flags = hf["video_flags"][:]
                self.groups = hf["groups"][:]
            ...
            for scene in tqdm(scene_dirs):
                ...
                subscenes = sorted(
                    [
                        ...
                        if d.startswith("dense_") # dense_0, dense_1
                    ]
                )
                id_range_subscenes = []
                for subscene in subscenes:
                    rgb_paths = sorted(
                        [
                            ...
                            if (d.endswith(".jpg") and os.path.exists(os.path.join(self.ROOT, scene, subscene, "depth", d.replace('jpg', 'npy'))))
                        ]
                    )
                    # st()
                    assert (
    ...
    def _get_views(self, idx, resolution, rng, num_views):
        ...
        for v, view_idx in enumerate(image_idxs):
            ...
            cam_path = img_path.replace("rgb", "cam").replace(".jpg", ".safetensor")
            ...
            camera_params = load_file(cam_path)
            ...
            threshold = (
                np.percentile(depthmap[depthmap > 0], 90) # less noisy
