...
import random
import itertools
from stream3r.dust3r.datasets.base.easy_dataset import EasyDataset # here we use fast's EasyDataset
from stream3r.dust3r.datasets_cut3r.utils.transforms import ImgNorm, SeqColorJitter
from stream3r.dust3r.utils.geometry import depthmap_to_absolute_camera_coordinates
import stream3r.dust3r.datasets_cut3r.utils.cropping as cropping
from stream3r.dust3r.datasets_cut3r.utils.corr import extract_correspondences_from_pts3d
...
class BaseMultiViewDataset(EasyDataset):
    ...
    def __getitem__(self, idx):
        ...
        for v, view in enumerate(views):
            ...
            view["pts3d"] = pts3d
            view["valid_mask"] = valid_mask & np.isfinite(pts3d).all(axis=-1) & (not blk_img_flag)

            # TODO, add black check.
            if blk_img_flag:
        ...
        if self.n_corres > 0: # for mast3r training.
            ref_view = views[0]
            for view in views:
