...
class ScanNet_Multi(BaseMultiViewDataset):
    ...
    def _load_data(self, split):
        self.scene_root = osp.join(
            self.ROOT, "scans" if split == "train" else "scans_test"
        )
        self.scenes = [
            scene for scene in os.listdir(self.scene_root) if scene.startswith("scene")
        ]

        if os.path.exists(osp.join(self.ROOT, f'{split}-pre-calculated-loaddata-{self.num_views}.pkl')):
            with open(osp.join(self.ROOT, f'{split}-pre-calculated-loaddata-{self.num_views}.pkl'), 'rb') as f:
            ...
            for scene in tqdm(self.scenes):
                ...
                if not os.path.exists(osp.join(scene_dir, "new_scene_metadata.pkl")):
                    continue
                with open(osp.join(scene_dir, "new_scene_metadata.pkl"), 'rb') as f:
                    data = pickle.load(f)
                # with np.load(
            ...
            with open(osp.join(self.ROOT, f'{split}-pre-calculated-loaddata-{self.num_views}.pkl'), 'wb') as f:
                pickle.dump(
