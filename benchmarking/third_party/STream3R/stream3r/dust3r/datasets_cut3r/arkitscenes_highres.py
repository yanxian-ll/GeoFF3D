...
class ARKitScenesHighRes_Multi(BaseMultiViewDataset):
    ...
    def _load_data(self, split):
        ...
        cache_file = f'{split}-pre-calculated-loaddata-{self.num_views}.pkl'
        if os.path.exists(osp.join(self.ROOT, cache_file)):
            with open(osp.join(self.ROOT, cache_file), 'rb') as f:
        ...
        for scene in all_scenes:
            ...
            # with np.load(osp.join(scene_dir, "scene_metadata.safetensor.npz")) as data:
        ...
        with open(osp.join(self.ROOT, cache_file), 'wb') as f:
            pickle.dump(
