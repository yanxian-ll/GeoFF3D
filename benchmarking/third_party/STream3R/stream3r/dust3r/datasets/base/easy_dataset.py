...
from stream3r.dust3r.datasets.base.batched_sampler import BatchedRandomSampler
...
class ResizedDataset(EasyDataset):
    ...
    def set_epoch(self, epoch):
        ...
        # we modify to depend on date too
        from datetime import datetime
        current_date = datetime.now()
        date_int = int(current_date.strftime("%Y%m%d"))
        rng = np.random.default_rng(seed=epoch + date_int + 777)
