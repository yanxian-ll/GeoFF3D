# -*- coding: utf-8 -*-
"""In-source VGGT-SLAM export utilities.

This module keeps benchmark post-processing inside the VGGT-SLAM source tree:
1) collect sampled predicted points from the solved submaps;
2) load sampled GT cameras/points from the scene;
3) align prediction to GT with pose-Sim3;
4) save standard eval files and a Rerun .rrd.

The outer benchmark runner should only call the source script and then compute
metrics from eval/pred_cameras.npz and eval/pred_points.ply.
"""
...
def resize_rgb_depth_K(
    ...
    # First align K to the depth resolution if cam metadata resolution differs.
    ...
    # Then resize RGB/depth/K to the benchmark target resolution.
...
def depth_pixels_to_world_points_numpy(
    depth: np.ndarray,
    K: np.ndarray,
    T_c2w: np.ndarray,
    rows: np.ndarray,
    ...
    """Back-project only sampled valid pixels, instead of the whole depth map."""
...
def prepared_stem_from_path(path: Path) -> Tuple[int, str]:
    stem = Path(path).stem
    m = re.match(r"^(\d+?)__(.+)$", stem)
    if m:
        return int(m.group(1)), m.group(2)
    m = re.search(r"\d+(?:\.\d+)?", stem)
    frame_idx = int(float(m.group())) if m else -1
    return frame_idx, stem


def build_frame_to_stem(image_names: Sequence[str]) -> Dict[int, str]:
    mapping: Dict[int, str] = {}
    for path in image_names:
        idx, stem = prepared_stem_from_path(Path(path))
        if idx >= 0:
            mapping[idx] = stem
    return mapping


def collect_pred_cameras_from_solver(solver, image_names: Sequence[str]) -> List[Dict[str, object]]:
            ...
            cams.append(
                {
                    ...
                    "T_c2w": np.asarray(pose, dtype=np.float32).reshape(4, 4),
                }
            )
...
def load_gt_artifacts(
    ...
    if int(max_gt_points) > 0 and len(depth_eligible) > 0:
        per_frame_cap = max(1, int(np.ceil(float(max_gt_points) * 1.5 / float(len(depth_eligible)))))
    else:
        per_frame_cap = 0

    print(
        f"[VGGT-SLAM][GT] loading GT artifacts: "
    ...
    gt_cam_by_stem: Dict[str, Dict[str, object]] = {}
    ...
    for stem in cam_eligible:
        try:
            ...
            gt_cams.append({"stem": stem, "T_c2w": T_c2w})
    ...
    depth_iter = depth_eligible
    if tqdm is not None and len(depth_eligible) > 0:
        depth_iter = tqdm(
            depth_eligible,
            desc="[VGGT-SLAM][GT]",
            total=len(depth_eligible),
            dynamic_ncols=True,
        )
    ...
    for i, stem in enumerate(depth_iter):
        ...
        try:
            ...
            # Match spatial pipeline: use the same benchmark target resolution.
            if target_h is None or target_w is None:
                fallback_h, fallback_w = compute_target_hw(
                ...
                fallback_h, fallback_w = int(target_h), int(target_w)
            ...
            num_valid = int(rows.shape[0])
            if num_valid <= 0:
                continue

            if int(per_frame_cap) > 0 and num_valid > int(per_frame_cap):
                rng = np.random.default_rng(int(seed) + 104729 * (i + 1))
                sel = rng.choice(num_valid, size=int(per_frame_cap), replace=False)
                rows = rows[sel]
                cols_px = cols_px[sel]
            ...
            print(f"[WARN] failed to load GT depth/points for {stem}: {exc}")
    ...
    print(
        f"[VGGT-SLAM][GT] loaded GT artifacts: "
...
def save_eval_outputs(eval_dir: Path, pred_cams, gt_cams, pred_points, pred_colors, gt_points, gt_colors, meta):
    ...
    print(f"[VGGT-SLAM][EXPORT] saved eval outputs: {eval_dir}")
...
def write_rrd(output_rrd: Path, pred_cams, pred_points, pred_colors, gt_cams, gt_points, gt_colors, app_id="vggt_slam_source_export"):
    import rerun as rr
    ...
    try:
        ...
        pass
    print(f"[VGGT-SLAM][EXPORT] saved Rerun recording: {output_rrd}")
...
def export_solver_outputs(
    ...
    pred_points, pred_cams, align_meta = align_prediction_to_gt_pose_sim3(pred_points, pred_cams, gt_cams)
    ...
    meta = {
        ...
        "script": "third_party/vggt-slam/main.py",
        "method": method,
        ...
        "point_stride": int(point_stride),
        "gt_resize_policy": {
            "compatible_with": "scripts/predict_scene_to_rrd_spatial.py",
            "target_h": int(gt_target_h) if gt_target_h is not None else None,
            "target_w": int(gt_target_w) if gt_target_w is not None else None,
            "max_side": int(max_side),
            "size_multiple": int(size_multiple),
            "note": "GT RGB/depth/K are resized before GT point construction, matching spatial pipeline.",
        },
    }
    save_eval_outputs(Path(eval_dir), pred_cams, gt_cams, pred_points, pred_colors, gt_points, gt_colors, meta=meta)
    write_rrd(Path(output_rrd), pred_cams, pred_points, pred_colors, gt_cams, gt_points, gt_colors)
    return meta
