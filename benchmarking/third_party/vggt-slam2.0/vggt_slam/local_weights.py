"""
Local weight loading utilities for VGGT-SLAM 2.0.
...
_VGGT_SLAM_DIR = _LOCAL_WEIGHTS_DIR.parent  # third_party/vggt-slam2.0
    ...
    def offline_torch_hub_load(repo_or_dir, model, *args, **kwargs):
        if repo_or_dir == "facebookresearch/dinov2" and local_dino_repo:
            ...
            if dino_path and dino_path.is_dir():
                print(
                    "[VGGT-SLAM2] Redirect DINOv2 torch.hub.load to local repo: "
                    f"{dino_path}"
                )
            ...
            if salad_path and salad_path.is_dir():
                print(
                    "[VGGT-SLAM2] Redirect SALAD torch.hub.load to local repo: "
                    f"{salad_path}"
                )
            ...
            raise RuntimeError(
                f"VGGT-SLAM2 offline mode is enabled, but no local repo is "
...
def load_vggt_weights(model, model_path=None, offline=None):
    ...
    if ckpt_path:
        print(f"[VGGT-SLAM2] Loading VGGT-1B weights from local path: {ckpt_path}")
        ...
        print(f"[VGGT-SLAM 2.0] Checkpoint load result: {incompatible}")
        return model
    ...
    print(
        "[VGGT-SLAM2] Local VGGT weights not found. "
...
def ensure_salad_ckpt(ckpt_path=None, offline=None):
    ...
    if resolved:
        print(f"[VGGT-SLAM2] Loading SALAD checkpoint from local path: {resolved}")
        return resolved
    ...
    print(
        "[VGGT-SLAM2] Local dino_salad.ckpt not found. "
