...
def _to_scalar(value):
    """Convert a scalar-like Python, NumPy, or Torch value to a Python float."""
    if torch.is_tensor(value):
        if value.numel() != 1:
            raise ValueError(f"Expected a single metric value, got tensor shape {tuple(value.shape)}")
        return float(value.detach().cpu().item())

    array = np.asarray(value)
    if array.size != 1:
        raise ValueError(f"Expected a single metric value, got array shape {array.shape}")
...
def benchmark(args):
    ...
    for benchmark_dataset_name, data_loader in data_loaders.items():
        ...
        for metric in across_dataset_results.keys():
            across_dataset_results[metric] = float(
                np.mean(across_dataset_results[metric])
            )
    ...
    for metric in per_dataset_results[next(iter(per_dataset_results))].keys():
        ...
        average_results[metric] = float(np.mean(metric_values))
