...
"""
Absolute-world dense multi-view metric reconstruction benchmark.

This evaluator is designed for world-frame UAV reconstruction methods.

Key protocol:
1. GT pointmaps / poses are kept in the dataset world coordinate system.
2. Predicted pointmaps / poses are kept in the model output world coordinate system.
3. No conversion to GT view0 or Pred view0 is performed.
4. Optional global pred->GT Sim(3) alignment is supported:
   - alignment_mode=none   : no alignment, default
   - alignment_mode=points : Sim(3) from dense point correspondences
   - alignment_mode=pose   : Sim(3) from camera center correspondences
   - alignment_mode=pose_points : independent point/pose Sim(3) alignments
5. The output metric names are kept consistent with benchmark.py by reusing compute_set_metrics().
"""
...
# Direct script execution only adds this file's directory to sys.path.
...
from benchmarking.dense_n_view.benchmark import (
    solve_batch_sim3_from_correspondence_batches,
    solve_batch_scale_yaw_translation,
    solve_batch_points_yaw_similarity,
    solve_batch_pose_yaw_similarity,
)


log = logging.getLogger(__name__)

# Absolute metric keys that should be set to nan when an absolute-world failure is detected.
ABS_METRIC_KEYS = {
    "abs_pointmap_mae",
    "abs_pointmap_rmse",
    "abs_pose_ate",
...
def is_abs_metric_failure(metrics, max_abs_pose_ate=200.0, max_abs_pointmap_rmse=200.0, max_abs_chamfer=200.0):
    """
    Filter catastrophic finite outliers for no-align absolute-world evaluation.
    Unit is assumed to be meter if dataset world coordinates are metric.

    Do not use this to judge normal bad performance.
    It only removes obviously broken samples that would dominate the mean.
    """
    checks = [
        metrics.get("abs_pose_ate", np.nan),
        metrics.get("abs_pointmap_rmse", np.nan),
        metrics.get("abs_fused_pc_chamfer_l1", np.nan),
    ]

    # non-finite already behaves like invalid
    if any(not np.isfinite(x) for x in checks):
        return True

    if metrics.get("abs_pose_ate", 0.0) > max_abs_pose_ate:
        return True
    if metrics.get("abs_pointmap_rmse", 0.0) > max_abs_pointmap_rmse:
        return True
    if metrics.get("abs_fused_pc_chamfer_l1", 0.0) > max_abs_chamfer:
        return True
...
def build_model_batch_with_imperfect_translation_priors(
    batch,
    args,
    batch_index: int,
):
    """Create model-only translation priors while preserving pristine GT.

    The returned views are shallow copies. Geometry, camera poses, and all GT
    fields in ``batch`` remain unchanged and therefore remain the reference for
    both direct absolute evaluation and optional post-hoc pose alignment.
    """
    enabled = bool(_cfg_get(args, "prior_robustness.enabled", False))
    if not enabled:
        return batch

    std_xy = float(_cfg_get(args, "prior_robustness.std_xy", 0.0))
    std_z = float(_cfg_get(args, "prior_robustness.std_z", 0.0))
    missing_ratio = float(_cfg_get(args, "prior_robustness.missing_ratio", 0.0))
    retained_count = int(_cfg_get(args, "prior_robustness.retained_count", -1))
    input_target = str(
        _cfg_get(args, "prior_robustness.input_target", "world_translation")
    ).lower().strip()
    seed_offset = int(_cfg_get(args, "prior_robustness.seed_offset", 470001))
    if std_xy < 0.0 or std_z < 0.0:
        raise ValueError("prior_robustness std_xy/std_z must be non-negative")
    if not 0.0 <= missing_ratio < 1.0:
        raise ValueError("prior_robustness.missing_ratio must be in [0, 1)")

    translations = torch.stack(
        [
            view.get("camera_pose_trans", view["camera_pose"][..., :3, 3])
            for view in batch
        ],
        dim=1,
    )
    batch_size, num_views = translations.shape[:2]
    generator = torch.Generator(device=translations.device)
    generator.manual_seed(int(args.seed) + seed_offset + 104729 * int(batch_index))
    ...
    if retained_count >= 0:
        if retained_count > num_views:
            raise ValueError(
                f"retained_count={retained_count} exceeds num_views={num_views}"
            )
        num_missing = num_views - retained_count
    else:
        num_missing = min(num_views - 1, int(round(missing_ratio * num_views)))
    if num_missing > 0:
        scores = torch.rand(
            batch_size,
            num_views,
        ...
        missing = torch.topk(scores, k=num_missing, dim=1, largest=True).indices
        mask.scatter_(1, missing, False)

    model_batch = []
    for view_idx, view in enumerate(batch):
        model_view = dict(view)
        if input_target == "world_translation":
            model_view["world_translation"] = perturbed[:, view_idx]
            model_view["world_translation_mask"] = mask[:, view_idx]
        elif input_target == "camera_pose":
            perturbed_pose = view["camera_pose"].clone()
            perturbed_pose[..., :3, 3] = perturbed[:, view_idx]
            model_view["camera_pose"] = perturbed_pose
            model_view["camera_pose_trans"] = perturbed[:, view_idx]
            model_view["camera_pose_mask"] = mask[:, view_idx]
        else:
            raise ValueError(
                "prior_robustness.input_target must be world_translation or camera_pose"
            )
        model_batch.append(model_view)
    return model_batch
...
def solve_batch_pose_sim3(
    ...
    """
    Solve one pred->gt Sim(3) from matched camera centers.

    Alignment form:
        C_gt = s * R * C_pred + t

    Args:
        gt_pose_trans_list: list of (B,3), GT camera centers/translations in view0 frame
        pr_pose_trans_list: list of (B,3), predicted camera centers/translations in pred view0 frame
    ...
    Returns:
        ...
        sim3_inlier_ratio: (B,)
    """
    device = gt_pose_trans_list[0].device
    dtype = gt_pose_trans_list[0].dtype
    batch_size = gt_pose_trans_list[0].shape[0]
    n_views = len(gt_pose_trans_list)
    ...
    for b in range(batch_size):
        ...
        for v in range(n_views):
            ...
            good = (
                torch.isfinite(pr_c).all()
                and torch.isfinite(gt_c).all()
            )
            if good:
                pr_corr_all.append(pr_c)
                gt_corr_all.append(gt_c)
        ...
        gt_centered = gt_corr_all - gt_corr_all.mean(dim=0, keepdim=True)
        scene_scale = torch.median(torch.linalg.norm(gt_centered, dim=1))
        if not torch.isfinite(scene_scale) or scene_scale <= 1e-8:
            scene_scale = torch.median(torch.linalg.norm(gt_corr_all, dim=1))
        if not torch.isfinite(scene_scale) or scene_scale <= 1e-8:
            scene_scale = torch.tensor(1.0, device=device, dtype=dtype)

        inlier_thr = max(0.1 * float(scene_scale.item()), 1e-3)
        inlier_ratio = (resid < inlier_thr).float().mean()

        sim3_median_residual[b] = med_resid
        sim3_inlier_ratio[b] = inlier_ratio

        gt_scale = torch.median(torch.linalg.norm(gt_centered, dim=1))
        pr_al_centered = pr_al - pr_al.mean(dim=0, keepdim=True)
        pr_al_scale = torch.median(torch.linalg.norm(pr_al_centered, dim=1))
        ...
        valid = (
            ...
            and (pr_corr_all.shape[0] >= min_corr)
            and torch.isfinite(med_resid)
            and (float(med_resid.item()) <= max_median_residual_ratio * float(scene_scale.item()))
            and torch.isfinite(inlier_ratio)
            and (float(inlier_ratio.item()) >= min_inlier_ratio)
            and torch.isfinite(scale_ratio)
            and (float(scale_ratio.item()) <= max_scale_ratio)
...
def get_all_info_for_metric_computation(
    batch,
    preds,
    norm_mode="avg_dis",
    alignment_mode="none",
    alignment_prior_trans=None,
    alignment_prior_masks=None,
):
    """
    Absolute-world version.

    Returns the same structure as benchmark.py:
      gt_info,
      pr_info,
      valid_masks,
      gt_info_abs,
    ...
    Differences from benchmark.py:
      - No GT -> view0 transform.
      - No Pred -> predicted view0 transform.
      - alignment_mode=none is the default.
      - Optional points/pose Sim(3) is estimated directly in dataset world coordinates.
    """
    alignment_mode = str(alignment_mode).lower().strip()
    if alignment_mode in {"point", "dense", "dense_points"}:
        alignment_mode = "points"
    elif alignment_mode in {"camera", "camera_center", "camera_centers", "pose_sim3"}:
        alignment_mode = "pose"
    elif alignment_mode in {"points_pose", "separate", "separate_pose_points"}:
        alignment_mode = "pose_points"
    elif alignment_mode in {
        "pose_points1", "pose_points_yaw", "yaw", "yaw_translation",
        "scale_yaw_translation",
    }:
        alignment_mode = "pose_points_yaw"
    elif alignment_mode in {
        "prior_yaw", "prior_yaw_translation", "prior_scale_yaw_translation"
    }:
        alignment_mode = "prior_yaw"
    elif alignment_mode in {"prior_pose", "prior_sim3"}:
        alignment_mode = "prior_pose"
    elif alignment_mode in {"pose_yaw", "pose_scale_yaw_translation"}:
        alignment_mode = "pose_yaw"

    if alignment_mode not in {
        "points", "pose", "pose_points", "pose_points_yaw", "pose_yaw",
        "prior_yaw", "prior_pose", "none"
    }:
        raise ValueError(
            f"Unsupported alignment_mode={alignment_mode}. "
            "Expected points, pose, pose_yaw, pose_points, pose_points_yaw, "
            "prior_yaw, prior_pose, or none."
        )
    ...
    # --------------------------------------------------------
    # 1) Collect GT / Pred quantities directly in world frame
    # --------------------------------------------------------
    no_norm_gt_pts = []
    ...
    for i in range(n_views):
        # ---------------- GT: keep dataset world frame ----------------
        gt_pts_i = batch[i]["pts3d"]
        ...
        gt_pose_i = batch[i]["camera_pose"]
        gt_pose_rot.append(gt_pose_i[..., :3, :3])
        no_norm_gt_pose_trans.append(gt_pose_i[..., :3, 3])

        # ---------------- Pred: keep model output world frame ----------------
        pr_rot_i = quaternion_to_rotation_matrix(preds[i]["cam_quats"])
        pr_trans_i = preds[i]["cam_trans"]
        pr_pts_i = preds[i]["pts3d"]

        # Keep existing benchmark behavior:
        # if the model outputs metric_scaling_factor, decode predictions before evaluation.
        if "metric_scaling_factor" in preds[i]:
            sf_map = preds[i]["metric_scaling_factor"].unsqueeze(-1).unsqueeze(-1)
            pr_pts_i = pr_pts_i / sf_map
            pr_pts3d_cam_i = preds[i]["pts3d_cam"] / sf_map
        ...
        if "intrinsics" in preds[i]:
            ...
            pr_intr_i = recover_pinhole_intrinsics_from_ray_directions(
                preds[i]["ray_directions"]
            )
    ...
    # --------------------------------------------------------
    # 2) Pre-filter obvious outliers before optional Sim(3)
    # --------------------------------------------------------
    valid_masks = [
        valid_masks[i]
    ...
    # --------------------------------------------------------
    # 3) Optional global pred->gt Sim(3) in world coordinates
    # --------------------------------------------------------
    points_sim3 = None
    pose_sim3 = None
    if alignment_mode == "prior_yaw":
        if alignment_prior_trans is None:
            raise ValueError("alignment_mode=prior_yaw requires imperfect translation priors")
        prior_batches, pred_batches = [], []
        for b in range(batch_size):
            prior_b = torch.stack([x[b] for x in alignment_prior_trans], dim=0)
            pred_b = torch.stack([x[b] for x in no_norm_pr_pose_trans], dim=0)
            if alignment_prior_masks is not None:
                keep = torch.stack([x[b] for x in alignment_prior_masks], dim=0).bool()
                prior_b = prior_b[keep]
                pred_b = pred_b[keep]
            prior_batches.append(prior_b)
            pred_batches.append(pred_b)
        prior_sim3 = solve_batch_scale_yaw_translation(
            prior_batches, pred_batches, device=device, dtype=dtype, min_corr=1
        )
        # With zero retained priors there is intentionally no post alignment.
        # The identity result is still a valid evaluation transform rather
        # than an alignment failure that should erase all absolute metrics.
        prior_parts = list(prior_sim3)
        zero_corr = prior_parts[4] == 0
        if zero_corr.any():
            prior_parts[3] = prior_parts[3].clone()
            prior_parts[5] = prior_parts[5].clone()
            prior_parts[6] = prior_parts[6].clone()
            prior_parts[3][zero_corr] = True
            prior_parts[5][zero_corr] = 0.0
            prior_parts[6][zero_corr] = 1.0
        prior_sim3 = tuple(prior_parts)
        points_sim3 = prior_sim3
        pose_sim3 = prior_sim3
    if alignment_mode == "prior_pose":
        if alignment_prior_trans is None:
            raise ValueError("alignment_mode=prior_pose requires imperfect priors")
        prior_batches, pred_batches = [], []
        for b in range(batch_size):
            prior_b = torch.stack([x[b] for x in alignment_prior_trans], dim=0)
            pred_b = torch.stack([x[b] for x in no_norm_pr_pose_trans], dim=0)
            if alignment_prior_masks is not None:
                keep = torch.stack([x[b] for x in alignment_prior_masks], dim=0).bool()
                prior_b, pred_b = prior_b[keep], pred_b[keep]
            prior_batches.append(prior_b)
            pred_batches.append(pred_b)
        prior_pose_sim3 = solve_batch_sim3_from_correspondence_batches(
            prior_batches, pred_batches, device=device, dtype=dtype, min_corr=3
        )
        points_sim3 = prior_pose_sim3
        pose_sim3 = prior_pose_sim3
    if alignment_mode == "pose_yaw":
        pose_yaw_sim3 = solve_batch_pose_yaw_similarity(
            no_norm_gt_pose_trans, no_norm_pr_pose_trans
        )
        # Like pose Sim(3), one camera-center transform is shared by geometry
        # and poses; only roll/pitch correction is forbidden.
        points_sim3 = pose_yaw_sim3
        pose_sim3 = pose_yaw_sim3
    if alignment_mode in {"points", "pose_points", "pose_points_yaw"}:
        if alignment_mode == "pose_points_yaw":
            points_sim3 = solve_batch_points_yaw_similarity(
                no_norm_gt_pts, no_norm_pr_pts, valid_masks, max_samples_per_view=4096
            )
        else:
            points_sim3 = solve_batch_global_sim3(
    ...
    if alignment_mode in {"pose", "pose_points", "pose_points_yaw"}:
        if alignment_mode == "pose_points_yaw":
            pose_sim3 = solve_batch_pose_yaw_similarity(
                no_norm_gt_pose_trans, no_norm_pr_pose_trans
            )
        else:
            pose_sim3 = solve_batch_pose_sim3(
                gt_pose_trans_list=no_norm_gt_pose_trans,
                pr_pose_trans_list=no_norm_pr_pose_trans,
                trim_ratio=1.0,
                iters=1,
    ...
    if alignment_mode == "none":
        # Absolute no-alignment protocol.
        identity_sim3 = (
            torch.ones(batch_size, device=device, dtype=dtype),
            torch.eye(3, device=device, dtype=dtype).unsqueeze(0).repeat(batch_size, 1, 1),
        ...
        points_sim3 = identity_sim3
        pose_sim3 = identity_sim3

    geometry_sim3 = points_sim3 if points_sim3 is not None else pose_sim3
    camera_sim3 = pose_sim3 if pose_sim3 is not None else points_sim3
    (
        geometry_scale, geometry_rot, geometry_trans, geometry_valid,
        geometry_num_corr, geometry_median_residual, geometry_inlier_ratio,
    ) = geometry_sim3
    (
        pose_scale, pose_rot, pose_trans, pose_valid,
        pose_num_corr, pose_median_residual, pose_inlier_ratio,
    ) = camera_sim3

    # --------------------------------------------------------
    # 4) Apply optional Sim(3) to predicted world quantities
    # --------------------------------------------------------
    aligned_pr_pts = []
    ...
    for i in range(n_views):
        pr_pts_i = no_norm_pr_pts[i]
        s_map = geometry_scale[:, None, None, None]
        t_map = geometry_trans[:, None, None, :]

        pr_pts_i_al = s_map * torch.einsum("bij,bhwj->bhwi", geometry_rot, pr_pts_i) + t_map
        aligned_pr_pts.append(pr_pts_i_al)

        # camera-frame depth points: only scale is meaningful.
        pr_pts3d_cam_i_al = no_norm_pr_pts3d_cam[i] * s_map
        aligned_pr_pts3d_cam.append(pr_pts3d_cam_i_al)

        pr_rot_i_al = torch.einsum("bij,bjk->bik", pose_rot, pr_pose_rot[i])
        pr_trans_i_al = pose_scale[:, None] * torch.einsum(
            "bij,bj->bi", pose_rot, no_norm_pr_pose_trans[i]
        ) + pose_trans
    ...
    # --------------------------------------------------------
    # 5) Build normalized info for relative metrics.
    #    This keeps metric names consistent with benchmark.py,
    #    but does NOT change world-frame absolute metrics.
    # --------------------------------------------------------
    ...
    for i in range(n_views):
        ...
        pose_norm_factor = (
            gt_norm_factor
            if alignment_mode in {"pose_points", "pose_points_yaw"}
            else pr_norm_factor
        )
        pr_pose_trans.append((aligned_pr_pose_trans[i] / pose_norm_factor[:, :, 0, 0]).cpu())

        valid_masks[i] = valid_masks[i].cpu()

    # --------------------------------------------------------
    # 6) Build pose matrices
    # --------------------------------------------------------
    gt_poses = []
    pr_poses = []
    gt_poses_abs = []
    pr_poses_abs = []
    pr_poses_abs_unaligned = []
    ...
    for i in range(n_views):
        ...
        pr_pose_abs_unaligned_curr = torch.eye(4, device=device, dtype=dtype).unsqueeze(0).repeat(batch_size, 1, 1)
        pr_pose_abs_unaligned_curr[..., :3, :3] = pr_pose_rot[i]
        pr_pose_abs_unaligned_curr[..., :3, 3] = no_norm_pr_pose_trans[i]
        pr_poses_abs_unaligned.append(pr_pose_abs_unaligned_curr.cpu())
    ...
    pr_info_abs_aligned = {
        ...
        "intrinsics": pr_intrinsics,
        "poses": pr_poses_abs,
        "poses_unaligned": pr_poses_abs_unaligned,
    ...
    scale_factors = {
        ...
        "pr_to_gt_scale": geometry_scale.detach().cpu(),
        "sim3_rot": geometry_rot.detach().cpu(),
        "sim3_trans": geometry_trans.detach().cpu(),
        "sim3_valid": geometry_valid.detach().cpu(),
        "sim3_num_corr": geometry_num_corr.detach().cpu(),
        "sim3_median_residual": geometry_median_residual.detach().cpu(),
        "sim3_inlier_ratio": geometry_inlier_ratio.detach().cpu(),
        "points_sim3_scale": geometry_scale.detach().cpu(),
        "points_sim3_rot": geometry_rot.detach().cpu(),
        "points_sim3_trans": geometry_trans.detach().cpu(),
        "points_sim3_valid": geometry_valid.detach().cpu(),
        "pose_sim3_scale": pose_scale.detach().cpu(),
        "pose_sim3_rot": pose_rot.detach().cpu(),
        "pose_sim3_trans": pose_trans.detach().cpu(),
        "pose_sim3_valid": pose_valid.detach().cpu(),
...
def benchmark(args):
    ...
    print("[absolute-world benchmark] No ref0 transform. Default alignment_mode=none.")
    if args.output_dir:
    ...
    robustness_meta = {
        ...
        "missing_ratio": float(_cfg_get(args, "prior_robustness.missing_ratio", 0.0)),
        "retained_count": int(_cfg_get(args, "prior_robustness.retained_count", -1)),
        "input_target": str(_cfg_get(args, "prior_robustness.input_target", "world_translation")),
        "seed_offset": int(_cfg_get(args, "prior_robustness.seed_offset", 470001)),
        "evaluation_reference": "original_unperturbed_gt",
        "translation_prior_perturbed": True,
        "rotation_prior_perturbed": False,
        "model_input": (
            f"perturbed_{_cfg_get(args, 'prior_robustness.input_target', 'world_translation')}_and_mask"
            if bool(_cfg_get(args, "prior_robustness.enabled", False))
            else "unchanged_batch"
        ),
    }
    if args.output_dir:
        robustness_path = Path(args.output_dir) / "prior_robustness_config.json"
        robustness_path.write_text(
            json.dumps(robustness_meta, indent=2, ensure_ascii=False),
            encoding="utf-8",
        )
    if robustness_meta["enabled"]:
        print(f"[prior robustness] {robustness_meta}")
    ...
    compute_separate_align_pose_ate = bool(_cfg_get(args, "compute_separate_align_pose_ate", False))
    if compute_separate_align_pose_ate and not bool(_cfg_get(args, "compute_abs_metrics", False)):
        print(
            "[benchmark] compute_separate_align_pose_ate=True but "
            "compute_abs_metrics=False; the separately aligned Pose ATE will not be computed."
        )
    ...
    per_dataset_results = {}
    datasets_to_run = {}
    requested_alignment = str(_cfg_get(args, "alignment_mode", "none")).lower().strip()
    protocol_path = os.path.join(args.output_dir, "alignment_protocol.json")
    cached_alignment = None
    if os.path.isfile(protocol_path):
        try:
            with open(protocol_path, "r", encoding="utf-8") as f:
                cached_alignment = str(json.load(f).get("alignment_mode", ""))
        except Exception:
            cached_alignment = None
    ...
    for benchmark_dataset_name, dataset_cfg in dataset_name_to_cfg.items():
        ...
        if (
            os.path.exists(per_scene_json_path)
            and os.path.exists(avg_json_path)
            and cached_alignment == requested_alignment
        ):
            with open(avg_json_path, "r", encoding="utf-8") as f:
                cached_avg = json.load(f)

            missing_requested_metrics = "camera_scale_rel_error" not in cached_avg
            if compute_separate_align_pose_ate:
                missing_requested_metrics = missing_requested_metrics or (
                    "abs_pose_ate_separate_aligned" not in cached_avg
                )

            if missing_requested_metrics:
                print(
                    f"Rerun dataset {benchmark_dataset_name}: cache exists but does not contain "
                    "the currently required scale/pose metrics."
                )
                ...
                per_dataset_results[benchmark_dataset_name] = cached_avg
        else:
            datasets_to_run[benchmark_dataset_name] = dataset_cfg
    ...
    for benchmark_dataset_name, dataset_cfg in datasets_to_run.items():
        ...
        for dataset_scene in data_loader.dataset.dataset.scenes:
            per_scene_results[dataset_scene] = {
                ...
                # Scale from raw corresponding camera centers (no alignment).
                "camera_scale_gt": [],
                "camera_scale_pred": [],
                "camera_scale_ratio": [],
                "camera_scale_rel_error": [],
                "camera_scale_log_error": [],
                ...
                # absolute-world evaluation validity
                "eval_valid": [],
            }
            if compute_separate_align_pose_ate:
                per_scene_results[dataset_scene]["abs_pose_ate_separate_aligned"] = []
        ...
        for batch_index, batch in enumerate(tqdm(data_loader)):
            ...
            model_batch = build_model_batch_with_imperfect_translation_priors(
                batch,
                args,
            ...
            alignment_mode = _cfg_get(args, "alignment_mode", "none")
            uses_imperfect_prior_alignment = str(alignment_mode).lower().strip() in {
                "prior_yaw", "prior_yaw_translation", "prior_scale_yaw_translation",
                "prior_pose", "prior_sim3",
            }
            alignment_input_target = str(
                _cfg_get(args, "prior_robustness.input_target", "world_translation")
            ).lower().strip()
            if uses_imperfect_prior_alignment:
                if alignment_input_target == "camera_pose":
                    alignment_trans = [view["camera_pose"][..., :3, 3] for view in model_batch]
                    alignment_masks = [view["camera_pose_mask"] for view in model_batch]
                else:
                    alignment_trans = [view["world_translation"] for view in model_batch]
                    alignment_masks = [view["world_translation_mask"] for view in model_batch]
            else:
                alignment_trans = None
            ...
            gt_info, pr_info, valid_masks, gt_info_abs, pr_info_abs_aligned, scale_factors = (
                get_all_info_for_metric_computation(
                    ...
                    preds,
                    alignment_mode=alignment_mode,
                    alignment_prior_trans=alignment_trans,
                    alignment_prior_masks=alignment_masks,
                )
            )
            ...
            for batch_idx in range(batch_size):
                ...
                metrics, fused_debug = compute_set_metrics(
                    ...
                    compute_separate_align_pose_ate=compute_separate_align_pose_ate,
                )
                raw_gt_centers = torch.stack(
                    [view["camera_pose"][batch_idx, :3, 3] for view in batch]
                )
                raw_pred_centers = torch.stack(
                    [pred["cam_trans"][batch_idx] for pred in preds]
                )
                metrics.update(
                    camera_center_scale_metrics(raw_gt_centers, raw_pred_centers)
                )
                ...
                # Absolute-world failure filtering for no-align mode
                alignment_mode = str(_cfg_get(args, "alignment_mode", "none")).lower().strip()
                filter_abs_failures = bool(_cfg_get(args, "filter_abs_failures", True))

                if alignment_mode == "none" and filter_abs_failures:
                    failed = is_abs_metric_failure(
                        metrics,
                        max_abs_pose_ate=float(_cfg_get(args, "max_abs_pose_ate", 200.0)),
                        max_abs_pointmap_rmse=float(_cfg_get(args, "max_abs_pointmap_rmse", 200.0)),
                        max_abs_chamfer=float(_cfg_get(args, "max_abs_chamfer", 200.0)),
                    )
                else:
                    failed = False

                metrics["eval_valid"] = 0.0 if failed else 1.0

                if failed:
                    for k in ABS_METRIC_KEYS:
                        if k in metrics:
                            metrics[k] = float("nan")
        ...
        eval_valid_mean = float(across_dataset_results.get("eval_valid", np.nan))
        if np.isfinite(eval_valid_mean):
            across_dataset_results["eval_failure_rate"] = 1.0 - eval_valid_mean
        else:
            across_dataset_results["eval_failure_rate"] = float("nan")
    ...
    if len(per_dataset_results) > 0:
        ...
        eval_valid_mean = float(average_results.get("eval_valid", np.nan))
        if np.isfinite(eval_valid_mean):
            average_results["eval_failure_rate"] = 1.0 - eval_valid_mean
        else:
            average_results["eval_failure_rate"] = float("nan")
    ...
    with open(protocol_path, "w", encoding="utf-8") as f:
        json.dump({"alignment_mode": requested_alignment}, f, indent=2)
