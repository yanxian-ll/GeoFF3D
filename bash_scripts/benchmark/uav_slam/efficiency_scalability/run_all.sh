...
"$SCRIPT_DIR/geoff3d.sh" "$CUDA_DEVICE"
"$SCRIPT_DIR/vggt_slam2.0.sh" "$CUDA_DEVICE"
"$SCRIPT_DIR/lingbot-map.sh" "$CUDA_DEVICE"

python3 "$SCRIPT_DIR/plot_scalability.py" \
  --results-root "$ROOT_DIR/outputs/efficiency_scalability" \
  --output "$ROOT_DIR/outputs/efficiency_scalability/efficiency_scalability" \
