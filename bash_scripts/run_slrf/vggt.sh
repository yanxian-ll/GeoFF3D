...
OUTPUT_PATH="${2:-$ROOT_DIR/experiments/uav_slrf/vggt/$SCENE_NAME}"
shift $(( $# >= 2 ? 2 : 1 ))

exec python3 "$ROOT_DIR/scripts/run_slrf.py" \
  model=vggt \
  scene_dir="$SCENE_DIR" \
  checkpoint="${CHECKPOINT:-$ROOT_DIR/experiments/uav_training/vggt_finetuning_8v_4d_8ipg_2g/checkpoint-best.pth}" \
