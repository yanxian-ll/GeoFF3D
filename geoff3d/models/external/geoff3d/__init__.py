...
from geoff3d.models.external.pi3.models.pi3x import Pi3X
...
def _maybe_sparsify_depths_like_geoff3d(
    depths: torch.Tensor,
    sparse_depth_prob: float,
    sparsification_removal_percent: float,
) -> torch.Tensor:
    """Randomly remove valid depth pixels using GeoFF3D's sparse-depth policy.
    ...
    per-view behavior in GeoFF3D's `_encode_and_fuse_depths`.
    """
...
class GeoFF3DWrapper(torch.nn.Module):
    ...
    def forward(self, views: List[Dict]):
        ...
        if depth_prob > 0.0 and _all_have(views, "depthmap"):
            ...
            depths = _maybe_sparsify_depths_like_geoff3d(
                depths,
