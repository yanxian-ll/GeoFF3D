...
def _match_longest_submodule_prefix(name, submodule_configs):
    """Return the longest matching submodule prefix for a parameter/module name."""
    if not submodule_configs:
        return None

    matches = []
    for submodule in submodule_configs.keys():
        if name == submodule or name.startswith(submodule + ".") or name.startswith(submodule):
...
def get_parameter_groups(
    ...
    for name, param in model.named_parameters():
        ...
        # Determine the submodule this parameter belongs to (longest-prefix match)
        submodule_name = _match_longest_submodule_prefix(name, submodule_configs)
...
def adjust_learning_rate_by_step(
    ...
    """Adjust learning rate with AnySplat-style step warmup + cosine decay."""
    if submodule_configs is None:
        submodule_configs = {}
    ...
    for group_num, param_group in enumerate(optimizer.param_groups):
        ...
        if submodule_name in submodule_configs:
            config = submodule_configs[submodule_name]
            lr = config.get("lr", train_args.lr)
            warmup_steps = config.get("warmup_steps", train_args.warmup_steps)
            min_lr = config.get("min_lr", train_args.min_lr)
            schedule_type = config.get("schedule_type", train_args.schedule_type)
        else:
            lr = train_args.lr
