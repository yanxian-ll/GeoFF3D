...
from geoff3d.utils.misc import invalid_to_zeros
from geoff3d.utils.warnings import no_warnings
...
def normalize_pose_translation_distances(
    pose_translation_distances,
    return_norm_factor=False,
):
    """
    Normalize translation distances by the average non-zero distance.

    Args:
        pose_translation_distances (torch.Tensor):
            Tensor of size [B, V, 1].
        return_norm_factor (bool):
            Whether to return norm_factor of size [B].

    Returns:
        normalized_pose_translation_distances (torch.Tensor):
            Tensor of size [B, V, 1].
        norm_factor (torch.Tensor):
            Tensor of size [B], if return_norm_factor=True.
    """
    assert (
        pose_translation_distances.ndim == 3
        and pose_translation_distances.shape[2] == 1
    )

    pose_translation_distances_scalar = pose_translation_distances.squeeze(-1)  # [B, V]
    non_zero_pose_translation_distances = pose_translation_distances_scalar > 0  # [B, V]

    sum_of_all_views_pose_translation_distances = (
        pose_translation_distances_scalar.sum(dim=1)
    )  # [B]
    count_of_all_views_with_non_zero_pose_translation_distances = (
        non_zero_pose_translation_distances.sum(dim=1)
    )  # [B]

    norm_factor = sum_of_all_views_pose_translation_distances / (
        count_of_all_views_with_non_zero_pose_translation_distances + 1e-8
    )  # [B]

    norm_factor = norm_factor.clip(min=1e-8)

    normalized_pose_translation_distances = (
        pose_translation_distances
        / norm_factor.unsqueeze(-1).unsqueeze(-1)
    )  # [B, V, 1]

    output = (
        (normalized_pose_translation_distances, norm_factor)
        if return_norm_factor
        else normalized_pose_translation_distances
    )
