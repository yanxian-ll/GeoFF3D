...
"""
This utils script contains PORTAGE of wai-core io methods for GeoFF3D.
"""
...
from geoff3d.utils.wai.ops import (
    to_numpy,
)
from geoff3d.utils.wai.semantics import (
...
def _read_exr_with_openexr(fname: str | Path) -> np.ndarray:
    """Read EXR data via OpenEXR when OpenCV lacks EXR codec support."""
    try:
        import OpenEXR
    except ImportError as exc:
        raise RuntimeError(
            "OpenCV failed to read EXR and the OpenEXR Python package is not installed."
        ) from exc

    exr = OpenEXR.File(str(fname))
    channels = exr.channels()
    if not channels:
        raise ValueError(f"EXR file has no channels: {fname}")
    ...
    for names in (
        ...
        if set(names).issubset(channel_names):
            return np.stack(
                [np.asarray(channels[name].pixels, dtype=np.float32) for name in names],
                axis=-1,
            )

    preferred_scalar_channels = ("Y", "Z", "R", "depth", "Depth")
    for name in preferred_scalar_channels:
        if name in channels:
            return np.asarray(channels[name].pixels, dtype=np.float32)

    return np.stack(
        [
            np.asarray(channels[name].pixels, dtype=np.float32)
...
def _read_exr(
    ...
    if data is None:
        if not Path(fname).exists():
            raise FileNotFoundError(f"EXR file does not exist: {fname}")
        try:
            data = _read_exr_with_openexr(fname)
        except Exception as exc:
            raise RuntimeError(f"Failed to read EXR file: {fname}") from exc
    if fmt == "torch":
