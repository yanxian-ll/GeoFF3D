...
def initialize_geoff3d_model(high_level_config, device):
    """
    Initialize GeoFF3D model with three-tier fallback approach:
    ...
    Returns:
        torch.nn.Module: Initialized GeoFF3D model
    """
    import torch
    from huggingface_hub import hf_hub_download

    from geoff3d.models import init_model, GeoFF3D

    print("Initializing GeoFF3D model...")
    ...
    try:
        print("Loading GeoFF3D model from_pretrained...")
        model = GeoFF3D.from_pretrained(high_level_config["hf_model_name"]).to(
            device
        )
        print("Loading GeoFF3D model from_pretrained succeeded...")
        return model
...
def initialize_geoff3d_local(local_config, device):
    """Initialize a GeoFF3D model entirely from local resources.
    ...
    Returns:
        torch.nn.Module: GeoFF3D model moved to ``device`` and switched to ``eval()``.
