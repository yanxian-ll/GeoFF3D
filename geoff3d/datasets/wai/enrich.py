"""
enrich Dataset using WAI format data.
"""
...
class ENRICHWAI(A3DRealWAI):
    ...
    def _load_data(self):
        split_metadata_path = os.path.join(
            ...
            self.split,
            f"enrich_scene_list_{self.split}.npy",
        )
        ...
        if self.use_hfov_balanced_sampling:
            hfov_metadata_path = os.path.join(
                self.dataset_metadata_dir,
                self.split,
                f"enrich_scene_hfov_{self.split}.json",
            )
...
def get_parser():
    ...
    parser.add_argument(
        "-rd", "--root_dir", default="../../dataset/data/enrich", type=str
    )
    parser.add_argument(
        "-dmd",
        "--dataset_metadata_dir",
    ...
    parser.add_argument(
        "--save_rrd",
        type=str,
        default="./experiments/dataset_viz/enrich_viz.rrd",
        help="Output .rrd file path",
    )
    parser.add_argument(
        "--app_id",
        type=str,
        default="enrich_Dataloader",
...
from geoff3d.datasets.base.base_dataset import view_name
...
#     from geoff3d.datasets.base.base_dataset import view_name
#     from geoff3d.utils.image import rgb
#     from geoff3d.utils.viz import script_add_rerun_args
...
#     if args.viz:
#         rr.script_setup(args, "ENRICH_Dataloader")
