...
"""
Base class for GeoFF3D datasets.
"""
...
from geoff3d.datasets.base.easy_dataset import EasyDataset
from geoff3d.utils.cropping import (
...
class BaseDataset(EasyDataset):
    ...
    def __init__(
        ...
        image_transform_mode: str = "per_view",
        aug_crop: int = 0,
        seed: int = None,
        ...
        Args:
            ...
            image_transform_mode (str): How stochastic image transforms are shared across views.
                                        Options:
                                        - "per_view": each view samples independent color augmentation.
                                        - "per_sample": all views in one sample reuse the same random
        ...
        self.image_transform_mode = str(image_transform_mode)
        if self.image_transform_mode not in {"per_view", "per_sample"}:
            raise ValueError(
                "image_transform_mode must be one of ['per_view', 'per_sample'], "
                f"got {self.image_transform_mode}"
            )
    ...
    def _apply_image_transform(self, image, shared_seed=None):
        """Apply image transform.

        If shared_seed is provided, stochastic torchvision transforms are seeded so
        that all views in the same sample receive the same augmentation parameters.

        This is useful for photometric/render training where multi-view color
        consistency matters. The default per-view mode keeps the original behavior.
        """
        if shared_seed is None:
            return self.transform(image)

        # torchvision random transforms use torch RNG. fork_rng restores the global
        # RNG state after applying the transform, avoiding side effects on the
        # dataloader worker's random stream.
        with torch.random.fork_rng(devices=[]):
            torch.manual_seed(int(shared_seed))
            return self.transform(image)
    ...
    def _getitem_fn(self, idx):
        ...
        shared_image_transform_seed = None
        if self.image_transform_mode == "per_sample":
            shared_image_transform_seed = int(self._rng.integers(0, 2**31 - 1))
        ...
        for v, view in enumerate(views):
            ...
            view["img"] = self._apply_image_transform(
                view["img"],
                shared_seed=shared_image_transform_seed,
            )
