...
from geoff3d.datasets.base.batched_sampler import (
    BatchedMultiFeatureRandomSampler,
    DynamicBatchedMultiFeatureRandomSampler,
    HFOVBalancedBatchedMultiFeatureRandomSampler,
    HFOVBalancedDynamicBatchedMultiFeatureRandomSampler,
)
...
class EasyDataset:
    ...
    def make_sampler(
        ...
        # ---- hfov-balanced scene sampling branch (train-time only) ----
        use_hfov_balanced_sampling = bool(
            getattr(self, "use_hfov_balanced_sampling", False)
        )
        hfov_bin_to_scene_indices = getattr(self, "hfov_bin_to_scene_indices", None)
        hfov_sampling_enabled = (
            use_hfov_balanced_sampling
            and isinstance(hfov_bin_to_scene_indices, dict)
            and len(hfov_bin_to_scene_indices) > 0
        )
        ...
        if use_dynamic_sampler:
            ...
            if hfov_sampling_enabled:
                return HFOVBalancedDynamicBatchedMultiFeatureRandomSampler(
                    self,
                    pool_sizes=feature_pool_sizes,
                    hfov_bin_to_scene_indices=hfov_bin_to_scene_indices,
            ...
            if hfov_sampling_enabled:
                return HFOVBalancedBatchedMultiFeatureRandomSampler(
                    self,
                    batch_size,
                    feature_pool_sizes,
                    hfov_bin_to_scene_indices=hfov_bin_to_scene_indices,
