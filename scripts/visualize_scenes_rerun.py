...
What it logs to Rerun:
  ...
  - world/world_axes: optional large world-axis marker
...
def log_world_axes_marker(
    ...
    size = float(axis_size) if axis_size > 0 else max(float(bbox_diag) * float(axis_size_ratio), float(min_axis_size), 1e-6)
    offset = float(up_offset) if up_offset > 0 else size * float(up_offset_ratio)
    origin = bbox_center + parse_signed_axis(up_axis) * offset
    ...
    rr.log(
        ...
        rr.Points3D(
            positions=np.stack([origin_f32, x_end, y_end, z_end], axis=0),
            colors=np.stack([color_origin, color_x, color_y, color_z], axis=0),
            labels=["world axes marker", "+X", "+Y", "+Z"],
        ),
    )
    print(f"[INFO] world axes marker: origin={origin.tolist()} size={size:.6g} up_axis={up_axis} offset={offset:.6g}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--scene", type=str, required=True, help="Path to scene folder containing images/cams and optional depth.")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "When depth is unavailable, use the camera-center bbox instead."
        ),
    )
    parser.add_argument("--world_axes_size", type=float, default=0.0,
                        help="World-axis marker length in world units. <=0 uses bbox diagonal * --world_axes_size_ratio.")
    parser.add_argument("--world_axes_size_ratio", type=float, default=0.12)
    parser.add_argument("--world_axes_min_size", type=float, default=1e-4)
    parser.add_argument("--world_axes_up_axis", type=str, default="z", choices=["x", "y", "z", "-x", "-y", "-z"])
    parser.add_argument("--world_axes_offset", type=float, default=0.0,
                        help="Offset from bbox center along --world_axes_up_axis. <=0 uses size * --world_axes_offset_ratio.")
    parser.add_argument("--world_axes_offset_ratio", type=float, default=0.75)
    parser.add_argument("--world_axes_radius", type=float, default=0.0,
                        help="World-axis line radius. <=0 reuses --axis_radius.")
...
def main() -> None:
    ...
    print(
        f"[INFO] images={len(images)} cams={len(cam_files)} depths={len(depths)} "
        f"image+cam={len(image_cam_stems)} rgbd={len(depth_stems)}"
    )
    if depth_stems:
        print(f"[INFO] frames used for point cloud: {len(selected_stems)} / {len(depth_stems)}")
    ...
    if args.save_ply and pts.shape[0] > 0:
        if o3d is None:
