...
# -*- coding: utf-8 -*-
"""
Build Table 6.5 and plot Figure 6.5 for Chapter 6 hFOV-binned prior analysis.

Table 6.5:
  hFOV sensitivity reduction with priors on A3D-FA.

Figure 6.5:
  hFOV-binned prior analysis after UAV-domain adaptation on A3D-FA.
...
Example:
  python scripts/viz/table65_figure65_hfov_prior_analysis.py \
    --benchmarking experiments/mapanything/benchmarking \
    --views 8,16,24,32 \
    --output experiments/mapanything/benchmarking/figures_tables/table65_figure65
"""
...
# Default MA-FT-A3D prior-aware input settings. Multiple candidates are kept
# because different benchmark scripts may name the fine-tuned folders differently.
DEFAULT_ADAPTED_PRIOR_SETTINGS: "OrderedDict[str, SettingSpec]" = OrderedDict(
    [
        (
            "RGB",
            SettingSpec(
...
def parse_setting_specs(items: list[str] | None) -> "OrderedDict[str, SettingSpec]":
    """Parse input rows/curves. Defaults to MA-FT-A3D RGB/C/P/CP variants."""
    if not items:
        return DEFAULT_ADAPTED_PRIOR_SETTINGS.copy()
...
def metric_gap_reduction(rgb_sens: float, prior_sens: float) -> float:
    if not is_finite_number(rgb_sens) or not is_finite_number(prior_sens):
        return float("nan")
    rgb_sens = float(rgb_sens)
    if math.isclose(rgb_sens, 0.0, abs_tol=1e-12):
        return float("nan")
    return float((rgb_sens - float(prior_sens)) / rgb_sens)


def compute_table65_values(
    bin_stats: Mapping[str, Mapping[str, Mapping[str, Mapping[str, Any]]]],
    scene_values: list[SceneMetricValue],
    settings: "OrderedDict[str, SettingSpec]",
    metrics: "OrderedDict[str, MetricSpec]",
    table_metric_aliases: list[str],
    mean_mode: str,
    baseline_input: str,
) -> tuple[dict, list[dict]]:
    """
    table_values[input][metric_alias] = {mean, sensitivity, gap_reduction, n_bins}

    Sensitivity = (max error across hFOV bins - min error across hFOV bins) / mean error.
    Gap Reduction is computed per metric against the RGB baseline sensitivity, then
    averaged over the selected table metrics:
        (Sensitivity_RGB - Sensitivity_prior) / Sensitivity_RGB
    """
    ...
    # First pass: means and sensitivities.
    for setting in settings.keys():
        table_values[setting] = OrderedDict()
        for metric_alias in table_metric_aliases:
            metric = metrics[metric_alias]
    ...
    # Second pass: gap reduction relative to RGB/baseline input.
    for setting in settings.keys():
        metric_gaps: list[float] = []
        for metric_alias in table_metric_aliases:
            rgb_sens = table_values.get(baseline_input, {}).get(metric_alias, {}).get("sensitivity", float("nan"))
            cur_sens = table_values.get(setting, {}).get(metric_alias, {}).get("sensitivity", float("nan"))
            gap = metric_gap_reduction(rgb_sens, cur_sens)
            # Use 0.0 for the baseline row when self-comparing, which makes the
            # table easier to read while preserving NaN if the RGB sensitivity is unavailable.
            if setting == baseline_input and is_finite_number(rgb_sens):
                gap = 0.0
            table_values[setting][metric_alias]["gap_reduction"] = gap
            if setting != baseline_input and is_finite_number(gap):
                metric_gaps.append(float(gap))

        avg_gap = 0.0 if setting == baseline_input else mean_finite(metric_gaps)
        table_values[setting]["_gap_reduction_avg"] = avg_gap
            ...
            rows.append(
                {
                    ...
                    "metric_key": metric.key,
                    "mean_mode": mean_mode,
                    "metric_mean": table_values[setting][metric_alias]["mean"],
                    "sensitivity": table_values[setting][metric_alias]["sensitivity"],
                    "metric_gap_reduction": table_values[setting][metric_alias]["gap_reduction"],
                    "avg_gap_reduction": avg_gap,
...
def best_table65_masks(
    ...
    gap_vals = {
        setting: table_values.get(setting, {}).get("_gap_reduction_avg", float("nan"))
        for setting in table_values.keys()
    }
    finite_gaps = [v for v in gap_vals.values() if is_finite_number(v)]
    if not finite_gaps:
        best_gap: set[str] = set()
    else:
        best_v = max(finite_gaps)
        best_gap = {
            setting for setting, v in gap_vals.items()
            if is_finite_number(v) and math.isclose(float(v), best_v, rel_tol=1e-12, abs_tol=1e-12)
        }
    return masks, best_gap


def table65_metric_prefix(metric_alias: str) -> str:
    return {
        "ray": "Ray",
        "depth": "Depth",
        "pose": "Pose",
...
def build_table65_markdown(
    ...
    masks, best_gap = best_table65_masks(table_values, table_metric_aliases)
    headers = ["Input"]
    for metric_alias in table_metric_aliases:
        prefix = table65_metric_prefix(metric_alias)
        headers.extend([f"{prefix} Mean ↓", f"{prefix} Sens. ↓"])
    headers.append("Gap Reduction ↑")
    aligns = [":---"] + ["---:" for _ in headers[1:]]

    rows: list[str] = []
    if include_caption:
        rows.append("**Table 6.5. hFOV sensitivity reduction with priors on A3D-FA.**")
        rows.append("")
    ...
    for setting in table_values.keys():
        ...
        gap = table_values[setting].get("_gap_reduction_avg", float("nan"))
        gap_to_show = float(gap) * 100.0 if gap_as_percent and is_finite_number(gap) else gap
        gap_cell = fmt_number(gap_to_show, precision_gap, missing)
        if gap_as_percent and gap_cell != missing:
            gap_cell = f"{gap_cell}%"
        gap_cell = maybe_bold_md(gap_cell, bold_best and setting in best_gap)
        row.append(gap_cell)
        rows.append("| " + " | ".join(row) + " |")
    return "\n".join(rows)


def build_table65_latex(
    table_values: Mapping[str, Mapping[str, Mapping[str, float]]],
    metrics: "OrderedDict[str, MetricSpec]",
    table_metric_aliases: list[str],
    precision_sens: int,
    ...
    masks, best_gap = best_table65_masks(table_values, table_metric_aliases)
    n_numeric = len(table_metric_aliases) * 2 + 1
    lines: list[str] = []
    lines.append("\\begin{table}[t]")
    lines.append("\\centering")
    lines.append("\\caption{hFOV sensitivity reduction with priors on A3D-FA.}")
    lines.append("\\label{tab:hfov_sensitivity_reduction_with_priors}")
    ...
    header.append("Gap Reduction $\\uparrow$")
    ...
    for setting in table_values.keys():
        ...
        if gap_as_percent and gap_cell != missing:
            gap_cell = f"{gap_cell}\\%"
        gap_cell = maybe_bold_tex(gap_cell, bold_best and setting in best_gap, missing)
        row.append(gap_cell)
...
def write_table65_csv(
    ...
    fieldnames = ["input"]
    for metric_alias in table_metric_aliases:
        fieldnames.extend([f"{metric_alias}_mean", f"{metric_alias}_sensitivity", f"{metric_alias}_gap_reduction", f"{metric_alias}_n_bins"])
    fieldnames.append("gap_reduction_avg")
    with path.open("w", encoding="utf-8", newline="") as f:
        ...
        for setting in table_values.keys():
            ...
            for metric_alias in table_metric_aliases:
                ...
                row[f"{metric_alias}_gap_reduction"] = table_values[setting][metric_alias].get("gap_reduction", float("nan"))
                row[f"{metric_alias}_n_bins"] = table_values[setting][metric_alias].get("n_bins", 0)
            row["gap_reduction_avg"] = table_values[setting].get("_gap_reduction_avg", float("nan"))
            writer.writerow(row)
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Build Table 6.5 and Figure 6.5 hFOV-binned prior analysis after UAV-domain adaptation on A3D-FA.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--benchmarking", type=Path, default=Path("experiments/mapanything/benchmarking"))
    p.add_argument("--output", type=Path, default=None, help="Output directory. Default: <benchmarking>/figures_tables/table65_figure65")
    ...
    p.add_argument("--table-mean-mode", choices=["bin", "sample"], default="bin", help="Table 6.5 Mean values: average hFOV-bin means, or all scene/view samples.")
    p.add_argument("--table-metrics", type=str, default="ray,depth,chamfer", help="Metric aliases used in Table 6.5. Default excludes pose, while Figure 6.5 still plots pose.")
    p.add_argument("--baseline-input", type=str, default="RGB", help="Baseline input used for Gap Reduction, normally RGB.")
    ...
    p.add_argument("--precision-gap", type=int, default=1, help="Precision for Gap Reduction when shown as percent.")
    p.add_argument("--gap-as-ratio", action="store_true", help="Show Gap Reduction as a ratio instead of percentage.")
    ...
    p.add_argument("--table-stem", type=str, default="table65_hfov_sensitivity_reduction_with_priors")
    p.add_argument("--figure-stem", type=str, default="figure65_hfov_binned_prior_analysis")
    p.add_argument("--title", type=str, default="Figure 6.5. hFOV-binned prior analysis after UAV-domain adaptation on A3D-FA", help="Figure title. Use empty string to disable.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    output_dir = Path(args.output) if args.output is not None else root / "figures_tables" / "table65_figure65"
    ...
    table_metric_aliases = split_csv_like(args.table_metrics)
    if not table_metric_aliases:
        raise ValueError("--table-metrics cannot be empty")
    for alias in table_metric_aliases:
        if alias not in metrics:
            raise ValueError(f"Unknown table metric alias {alias!r}. Available: {list(metrics.keys())}")
    if args.baseline_input not in settings:
        raise ValueError(f"--baseline-input {args.baseline_input!r} not found in settings: {list(settings.keys())}")
    ...
    table_values, table_rows = compute_table65_values(
        bin_stats=bin_stats,
        scene_values=scene_values,
        settings=settings,
        metrics=metrics,
        table_metric_aliases=table_metric_aliases,
    ...
    write_table65_csv(table_csv_path, table_values, table_metric_aliases)
    write_dict_rows_csv(table_details_path, table_rows)

    table_metadata = {
        "table_id": "6.5",
        "caption": "hFOV sensitivity reduction with priors on A3D-FA.",
        ...
        "table_metrics": table_metric_aliases,
        "baseline_input": args.baseline_input,
        "agg": args.agg,
        "table_mean_mode": args.table_mean_mode,
        "sensitivity_formula": "(max error across hFOV bins - min error across hFOV bins) / mean error",
        "gap_reduction_formula": "(Sensitivity_RGB - Sensitivity_prior) / Sensitivity_RGB, averaged over table metrics",
        "outputs": {
    ...
    combined_metadata_path = output_dir / "table65_figure65_hfov_prior_analysis_metadata.json"
    combined_metadata = {
        "benchmarking_root": str(root),
    ...
    print(md)
    print(f"\nWrote Table 6.5 Markdown: {md_path}")
    print(f"Wrote Table 6.5 LaTeX:    {tex_path}")
    print(f"Wrote Table 6.5 CSV:      {table_csv_path}")
    print(f"Wrote Table 6.5 details:  {table_details_path}")
    print(f"Wrote Table 6.5 metadata: {table_metadata_path}")
    print(f"Wrote Figure 6.5 PNG:     {png_path}")
    print(f"Wrote Figure 6.5 PDF:     {pdf_path}")
    if args.save_svg:
        print(f"Wrote Figure 6.5 SVG:     {svg_path}")
