...
class StreamVGGTWrapper(torch.nn.Module):
    def __init__(
        self,
        name: str = "streamvggt",
    ...
    def run(
        ...
        *,
        ...
        del scene_dir
        repo_root = Path(__file__).resolve().parents[4] / "third_party" / "StreamVGGT"
        runner = Path(__file__).with_name("infer.py")
        output_dir = Path(output_dir).expanduser().resolve()
        output_dir.mkdir(parents=True, exist_ok=True)
        stdout_log_path = output_dir / "streamvggt_stdout.log"
