...
from mapanything.models.mapanything.head import CameraHead
...
class MapAnything(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        Args:
            ...
            additional_token_config (Dict): Configuration for additional tokens such as camera token and scale token. (default: None)
        ...
        self.additional_token_config = additional_token_config
        ...
        self.class_init_args = {
            ...
            "additional_token_config": self.additional_token_config,
        ...
        # Initialize the Additonal Token
        self._initialize_additional_tokens()
    ...
    def _initialize_additional_tokens(self):
        """
        Initialize additional tokens used by the multi-view transformer.

        Current additional tokens:
            - camera token: one per view, built from fused per-view features
            - scale token: one global token used by the existing metric-scale branch
        """
        scale_token_config = self.additional_token_config.get("scale", {})
        camera_token_config = self.additional_token_config.get("camera", {})

        # Scale token branch
        self.scale_token_enabled = bool(scale_token_config.get("enabled", False))
        if not self.scale_token_enabled:
            raise ValueError(
                "scale.enabled=False is not supported yet because the current "
                "MapAnything forward/prediction path assumes a scale token exists."
            )
        ...
        # Camera token branch
        self.camera_token_enabled = bool(camera_token_config.get("enabled", False))
        if self.camera_token_enabled:
            camera_token_hidden_dim = camera_token_config.get("hidden_dim")
            if camera_token_hidden_dim is None:
                camera_token_hidden_dim = self.encoder.enc_embed_dim

            self.camera_token_projector = nn.Sequential(
                nn.LayerNorm(self.encoder.enc_embed_dim),
                nn.Linear(self.encoder.enc_embed_dim, camera_token_hidden_dim),
                nn.GELU(),
                nn.Linear(camera_token_hidden_dim, self.encoder.enc_embed_dim),
            )
            self.camera_token_bias = nn.Parameter(
                torch.zeros(self.encoder.enc_embed_dim)
            )
            torch.nn.init.trunc_normal_(self.camera_token_bias, std=0.02)
        else:
            self.camera_token_projector = None
            self.camera_token_bias = None
    ...
    def _initialize_prediction_heads(self, pred_head_config):
        ...
        camera_head_config = pred_head_config.get("camera_head", {})
        self.camera_head_enabled = bool(camera_head_config.get("enabled", False))

        if self.camera_head_enabled:
            camera_head_input_dim = camera_head_config.get("input_dim")
            if camera_head_input_dim is None:
                camera_head_input_dim = self.info_sharing.dim

            camera_head_hidden_dim = camera_head_config.get("hidden_dim")
            if camera_head_hidden_dim is None:
                camera_head_hidden_dim = camera_head_input_dim

            self.camera_head = CameraHead(
                dim_in=camera_head_input_dim,
                hidden_dim=camera_head_hidden_dim,
                num_layers=int(camera_head_config.get("num_layers", 2)),
                predict_translation=bool(camera_head_config.get("predict_translation", False)),
                predict_rotation=bool(camera_head_config.get("predict_rotation", False)),
                predict_fov=bool(camera_head_config.get("predict_fov", True)),
            )
        else:
    ...
    def _compute_pose_quats_and_relative_trans_for_across_views(
        self,
        views,
        num_views,
        device,
        dtype,
        batch_size_per_view,
        per_sample_cam_rot_input_mask,
        per_sample_cam_trans_input_mask,
    ):
        """
        Split-camera-prior mode:
        - rot branch: same as legacy, compute relative quaternions in ref-view-0 frame
        - trans branch: compute relative translation vectors t_i - t_0

        Returns:
            pose_quats_across_views: (B*V, 4)
            pose_trans_across_views: (B*V, 3)
            per_sample_cam_rot_input_mask: updated valid mask
            per_sample_cam_trans_input_mask: updated valid mask
        """
        per_sample_cam_rot_input_mask = per_sample_cam_rot_input_mask.clone()
        per_sample_cam_trans_input_mask = per_sample_cam_trans_input_mask.clone()

        total_batch = batch_size_per_view * num_views

        pose_quats_across_views = torch.tensor(
            [0.0, 0.0, 0.0, 1.0], dtype=dtype, device=device
        ).repeat(total_batch, 1)

        pose_trans_across_views = torch.zeros(
            (total_batch, 3), dtype=dtype, device=device
        )
        ...
        rot_quats_ref0 = []
        for view_idx in range(num_views):
            start = view_idx * batch_size_per_view
            end = (view_idx + 1) * batch_size_per_view
            curr_mask = per_sample_cam_rot_input_mask[start:end]

            if "camera_pose_quats" in views[view_idx] and curr_mask.any():
                rot_quats_curr.append(views[view_idx]["camera_pose_quats"][curr_mask])
                rot_quats_ref0.append(views[0]["camera_pose_quats"][curr_mask])
            else:
                per_sample_cam_rot_input_mask[start:end] = False

        if len(rot_quats_curr) > 0:
            rot_quats_curr = torch.cat(rot_quats_curr, dim=0)
            rot_quats_ref0 = torch.cat(rot_quats_ref0, dim=0)

            rel_quats, _ = transform_pose_using_quats_and_trans_2_to_1(
                rot_quats_ref0,
                torch.zeros((rot_quats_ref0.shape[0], 3), dtype=dtype, device=device),
                rot_quats_curr,
                torch.zeros((rot_quats_curr.shape[0], 3), dtype=dtype, device=device),
            )
            pose_quats_across_views[per_sample_cam_rot_input_mask] = rel_quats.to(dtype=dtype)

        # ---------------------------
        # 2) relative-translation branch
        # ---------------------------
        for view_idx in range(num_views):
            start = view_idx * batch_size_per_view
            end = (view_idx + 1) * batch_size_per_view
            curr_mask = per_sample_cam_trans_input_mask[start:end]

            if curr_mask.any() and "camera_pose_trans" in views[view_idx]:
                curr_trans = views[view_idx]["camera_pose_trans"][curr_mask]
                ref_trans = views[0]["camera_pose_trans"][curr_mask]

                relative_trans = curr_trans - ref_trans
                pose_trans_across_views[start:end][curr_mask] = relative_trans.to(
                    dtype=dtype
                )
            else:
                per_sample_cam_trans_input_mask[start:end] = False
            ...
            if (
                ...
                if apply_mvs:
                    if "mvs_mask" in views[view_idx]:
                        ...
                        if mvs_mask_for_curr_view.dim() == 3:
                            mvs_mask_for_curr_view = mvs_mask_for_curr_view.unsqueeze(-1)

                        # Convert to multiplicative mask in {0,1} with same dtype/device
                        if mvs_mask_for_curr_view.dtype == torch.bool:
                            mvs_mask_for_curr_view = mvs_mask_for_curr_view.to(depth_for_curr_view_input.dtype)
                        else:
                            # if float mask, assume already 0/1 or [0,1]; binarize softly
                            mvs_mask_for_curr_view = (mvs_mask_for_curr_view > 0.5).to(depth_for_curr_view_input.dtype)
    ...
    def _encode_and_fuse_split_cam_quats_and_trans(
        self,
        views,
        num_views,
        batch_size_per_view,
        ...
        """
        Split-camera-prior mode:
        - rot branch: same as legacy
        - trans branch: native 3D translation encoder + norm_factor
        """
        ...
        # Zero out the pose quat features where the camera input mask is False
        pose_quats_features_across_views = (
            pose_quats_features_across_views * per_sample_cam_rot_input_mask.unsqueeze(-1)
        )
        ...
        # ----------------------------------
        # Native relative-translation branch
        # ----------------------------------
        pose_trans_features_across_views = torch.zeros(
            (batch_size_per_view * num_views, all_encoder_features_across_views.shape[1]),
            dtype=all_encoder_features_across_views.dtype,
            device=device,
        )
        pose_trans_scale_features_across_views = torch.zeros_like(
            pose_trans_features_across_views
        )

        if per_sample_cam_trans_input_mask.any():
            pose_trans = torch.split(
                pose_trans_across_views, batch_size_per_view, dim=0
            )
            pose_trans = torch.stack(pose_trans, dim=1)  # (B, V, 3)
            scaled_pose_trans, pose_trans_norm_factors = normalize_pose_translations(
                pose_trans, return_norm_factor=True
            )

            scaled_pose_trans = torch.cat(
                scaled_pose_trans.unbind(dim=1), dim=0
            )  # (B*V, 3)
            ...
            trans_feats = self.cam_trans_encoder(
                EncoderGlobalRepInput(data=scaled_pose_trans)
            ).features
            trans_feats = (
                trans_feats * per_sample_cam_trans_input_mask.unsqueeze(-1)
            )
            pose_trans_features_across_views = (
                pose_trans_features_across_views + trans_feats
            )

            trans_scale_feats = self.cam_trans_scale_encoder(
                EncoderGlobalRepInput(
                    data=torch.log(pose_trans_norm_factors_across_views + 1e-8)
                )
            ).features
            trans_scale_feats = (
                trans_scale_feats
                * per_sample_cam_trans_input_mask.unsqueeze(-1)
                * metric_scale_pose_trans_mask.unsqueeze(-1)
            )
            pose_trans_scale_features_across_views = (
                pose_trans_scale_features_across_views + trans_scale_feats
            )
    ...
    def _encode_and_fuse_optional_geometric_inputs(
        ...
        if "cam_rot_prob" in self.geometric_input_config or "cam_trans_prob" in self.geometric_input_config:
            cam_rot_prob = float(self.geometric_input_config.get("cam_rot_prob", 0.0))
            cam_trans_prob = float(self.geometric_input_config.get("cam_trans_prob", 0.0))

            per_sample_cam_rot_input_mask = (
                torch.rand(batch_size_per_view, device=device) < cam_rot_prob
            )
            per_sample_cam_rot_input_mask = per_sample_cam_rot_input_mask.repeat(num_views)
            per_sample_cam_rot_input_mask = (
                per_sample_cam_rot_input_mask & per_sample_geometric_input_mask
            )

            per_sample_cam_trans_input_mask = (
                torch.rand(batch_size_per_view, device=device) < cam_trans_prob
            )
            per_sample_cam_trans_input_mask = per_sample_cam_trans_input_mask.repeat(num_views)
            per_sample_cam_trans_input_mask = (
                per_sample_cam_trans_input_mask & per_sample_geometric_input_mask
            )

            pose_quats_across_views, pose_trans_across_views, per_sample_cam_rot_input_mask, per_sample_cam_trans_input_mask = (
                self._compute_pose_quats_and_relative_trans_for_across_views(
                    views,
                    num_views,
                    device,
                    dtype,
            ...
            all_encoder_features_across_views = (
                self._encode_and_fuse_split_cam_quats_and_trans(
                    views,
                    num_views,
                    batch_size_per_view,
                    all_encoder_features_across_views,
    ...
    def _build_per_view_camera_tokens(self, all_encoder_features_across_views):
        """
        Build one camera token per view from the fused encoder features.

        Each returned token is view-specific and keeps the original register-token
        layout convention of (B, C, T) with T=1.
        """
        per_view_camera_tokens = []
        for per_view_features in all_encoder_features_across_views:
            pooled_features = per_view_features.mean(dim=(-2, -1))
            camera_token = self.camera_token_projector(pooled_features)
            camera_token = camera_token + self.camera_token_bias.unsqueeze(0)
            per_view_camera_tokens.append(camera_token.unsqueeze(-1))
        return tuple(per_view_camera_tokens)

    def _append_camera_token_to_per_view_tokens(
        self,
        additional_input_tokens_per_view,
        per_view_camera_tokens,
        num_views,
    ):
        """
        Append the per-view camera tokens to each view's optional per-view tokens
        while preserving the existing token layout.
        """
        if per_view_camera_tokens is None:
            return additional_input_tokens_per_view

        if len(per_view_camera_tokens) != num_views:
            raise ValueError(
                "Number of per-view camera tokens must match the number of views."
            )

        if additional_input_tokens_per_view is None:
            return per_view_camera_tokens

        merged_tokens = []
        for existing_tokens, camera_token in zip(
            additional_input_tokens_per_view, per_view_camera_tokens
        ):
            if existing_tokens is None:
                merged_tokens.append(camera_token)
                continue

            if existing_tokens.ndim != 3:
                raise ValueError(
                    "Expected per-view tokens to have rank 3 when appending camera tokens."
                )

            if existing_tokens.shape[0] != camera_token.shape[0]:
                raise ValueError(
                    "Batch size mismatch between existing tokens and the camera token."
                )

            if existing_tokens.shape[1] == self.encoder.enc_embed_dim:
                merged_tokens.append(torch.cat([existing_tokens, camera_token], dim=-1))
            elif existing_tokens.shape[-1] == self.encoder.enc_embed_dim:
                merged_tokens.append(
                    torch.cat([existing_tokens, camera_token.transpose(1, 2)], dim=1)
                )
            else:
                raise ValueError(
                    "Could not infer token layout when appending the camera token."
                )
    ...
    def downstream_head(
        ...
        camera_head_inputs: torch.Tensor,
        img_shape: Tuple[int, int],
        ...
        Args:
            ...
            camera_head_inputs: Camera head input features of shape (B*V, C).
        ...
        if memory_efficient_inference:
            ...
            camera_outputs_list = [] if (self.camera_head_enabled and camera_head_inputs is not None) else None
            ...
            for batch_idx in range(num_batches):
                ...
                # Camera prediction
                if self.camera_head_enabled and camera_head_inputs is not None:
                    camera_head_inputs_batch = camera_head_inputs[start_idx:end_idx]
                    decoded_camera_batch = self.camera_head(
                        camera_head_inputs_batch.unsqueeze(1)
                    )
                    camera_outputs_list.append(decoded_camera_batch)
            ...
            # Concatenate camera outputs
            camera_final_outputs = None
            if self.camera_head_enabled and camera_head_inputs is not None:
                available_keys = decoded_camera_batch.__dict__.keys()
                camera_pred_data_dict = {
                    key: torch.cat(
                        [getattr(output, key) for output in camera_outputs_list],
                        dim=0,
                    )
                    for key in available_keys
                }
                camera_final_outputs = decoded_camera_batch.__class__(
                    **camera_pred_data_dict
                )
            ...
            camera_final_outputs = None
            if self.camera_head_enabled and camera_head_inputs is not None:
                camera_final_outputs = self.camera_head(
                    camera_head_inputs.unsqueeze(1)
                )
        ...
        return dense_final_outputs, pose_final_outputs, scale_final_output, camera_final_outputs
    ...
    def forward(self, views, memory_efficient_inference=False, minibatch_size=None):
        ...
        if self.camera_token_enabled:
            # Build per-view camera tokens from the fused per-view encoder features.
            per_view_camera_tokens = self._build_per_view_camera_tokens(all_encoder_features_across_views)

            # Append the per-view camera tokens to the per-view tokens for all views first,
            # so the token structure used by the transformer is finalized before any optional
            # auxiliary prediction is computed.
            all_encoder_registers_across_views = (
                self._append_camera_token_to_per_view_tokens(
                    all_encoder_registers_across_views,
                    per_view_camera_tokens,
                    num_views,
                )
        ...
        with torch.autocast("cuda", enabled=False):
            ...
            camera_head_inputs = None
            if self.camera_head_enabled and self.camera_token_enabled:
                transformed_additional_tokens_per_view = (
                    final_info_sharing_multi_view_feat.additional_token_features_per_view
                )

                camera_head_inputs_list = []
                for per_view_tokens in transformed_additional_tokens_per_view:
                    if per_view_tokens.ndim != 3:
                        raise ValueError(
                            "Expected per-view additional token features to have rank 3."
                        )

                    if per_view_tokens.shape[1] == self.encoder.enc_embed_dim:
                        # (B, C, T), camera token is appended last
                        camera_token_feat = per_view_tokens[:, :, -1]
                    elif per_view_tokens.shape[-1] == self.encoder.enc_embed_dim:
                        # (B, T, C), camera token is appended last
                        camera_token_feat = per_view_tokens[:, -1, :]
                    else:
                        raise ValueError(
                            "Could not infer token layout for camera head input."
                        )
                    camera_head_inputs_list.append(camera_token_feat)
                camera_head_inputs = torch.cat(camera_head_inputs_list, dim=0)

            # Run the downstream heads
            dense_final_outputs, pose_final_outputs, scale_final_output, camera_final_outputs = (
                self.downstream_head(
                    dense_head_inputs=dense_head_inputs,
                    scale_head_inputs=scale_head_inputs,
                    camera_head_inputs=camera_head_inputs,
                    img_shape=img_shape,
            ...
            if camera_final_outputs is not None:
                if "translation" in camera_final_outputs:
                    camera_translation_per_view = camera_final_outputs["translation"].squeeze(1).chunk(num_views, dim=0)
                    for i in range(num_views):
                        res[i]["camera_translation"] = camera_translation_per_view[i] * scale_final_output

                if "quaternion" in camera_final_outputs:
                    camera_quaternion_per_view = camera_final_outputs["quaternion"].squeeze(1).chunk(num_views, dim=0)
                    for i in range(num_views):
                        res[i]["camera_quaternion"] = camera_quaternion_per_view[i]

                if "fov_hw" in camera_final_outputs:
                    camera_fov_hw_per_view = camera_final_outputs["fov_hw"].squeeze(1).chunk(num_views, dim=0)
                    for i in range(num_views):
                        res[i]["camera_fov_hw"] = camera_fov_hw_per_view[i]
