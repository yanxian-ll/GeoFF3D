...
from mapanything.train.visualizers import build_tensorboard_visualizers
from mapanything.train.visualizers.base import VisualizationContext
from mapanything.peft.lora_utils import apply_lora_from_submodule_configs, has_enabled_lora_from_train_params, save_merged_checkpoint
...
def _set_loss_schedule_context(criterion, step: int, epoch: float) -> None:
    """Update optional per-step loss schedulers before a criterion forward."""
    if criterion is None:
        return
    for module in criterion.modules():
        set_step = getattr(module, "set_schedule_step", None)
        if callable(set_step):
            set_step(step=step, epoch=epoch)
...
def train(args):
    ...
    # Apply LoRA (selection is driven by train_params.submodule_configs)
    model = apply_lora_from_submodule_configs(
        model, args.train_params.submodule_configs
    )
    should_save_merged = has_enabled_lora_from_train_params(args.train_params)
    ...
    def save_model(epoch, fname, best_so_far):
        ...
        # Extra: save inference-ready merged checkpoint
    ...
    tensorboard_visualizers = build_tensorboard_visualizers(args, writer=log_writer)
        ...
        if (
            ...
            for test_name, testset in data_loader_test.items():
                ...
                stats = test_one_epoch(
                    ...
                    args=args,
                    prefix=test_name,
                    tensorboard_visualizers=tensorboard_visualizers,
                )
...
def train_one_epoch(
    ...
    for data_iter_step, batch in enumerate(
        ...
        if data_iter_step % accum_iter == 0:
            if getattr(args.train_params, "lr_schedule_unit", "epoch") == "step":
                train_tools.adjust_learning_rate_by_step(
                    optimizer,
                    global_step,
                    args.train_params.epochs * len(data_loader),
                    args.train_params,
        ...
        _set_loss_schedule_context(
            model.module if hasattr(model, "module") else model,
            step=global_step,
            epoch=epoch_f,
        )
        _set_loss_schedule_context(criterion, step=global_step, epoch=epoch_f)

        should_log_visuals = (
            tensorboard_visualizers is not None
            and tensorboard_visualizers.should_log(
                stage="train",
                step=global_step,
                epoch=epoch_f,
        ...
        result_or_loss = loss_of_one_batch_multi_view(
            batch,
            model,
            criterion,
            device,
            ...
            ret=None if should_log_visuals else "loss",
        )

        if should_log_visuals:
            loss_tuple = result_or_loss["loss"]
            tensorboard_visualizers.log(
                writer=log_writer,
                result=result_or_loss,
            ...
            loss_tuple = result_or_loss

        loss, loss_details = loss_tuple  # criterion returns two values
        if n_views > 2 and getattr(
            args.train_params, "scale_loss_by_num_views", True
        ):
            loss = loss * (
                2 / n_views
