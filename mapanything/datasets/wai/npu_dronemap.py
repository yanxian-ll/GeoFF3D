...
class NPUDroneMapWAI(A3DRealWAI):
    """
    ...
    def _load_data(self):
        split_metadata_path = os.path.join(
            ...
            self.split,
            f"npu_dronemap_scene_list_{self.split}.npy",
        )
        ...
        if self.use_hfov_balanced_sampling:
            hfov_metadata_path = os.path.join(
                self.dataset_metadata_dir,
                self.split,
                f"npu_dronemap_scene_hfov_{self.split}.json",
            )
...
def get_parser():
    ...
    parser.add_argument(
        "-rd", "--root_dir", default="../../dataset/data/npu_dronemap", type=str
    )
    parser.add_argument(
        "-dmd",
        "--dataset_metadata_dir",
